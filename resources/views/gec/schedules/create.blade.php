...
<div class="card">
    ...
    <form id="scheduleForm" method="POST" action="{{ route('gec.schedules.store') }}">
        ...
        <div class="form-grid">
            <div><label>Program / Degree</label><input class="input" value="{{ $department }}" disabled></div>
            <div>
            ...
            <div class="section-preview">
                <div class="section-preview-head"><div><h3>Sections the Generator Will Use</h3><p>Automatically selected from {{ $department }}'s existing Sections list.</p></div><span id="sectionCountStatus" class="section-count-status">0 sections available</span></div>
                <div id="sectionList" class="section-list">
                    ...
                    @endforeach
                    <p id="sectionEmpty" class="section-empty" hidden>No existing {{ $department }} sections match this academic year and year level.</p>
                </div>
            </div>
        </div>
...
<script>
    (() => {
        ...
        departmentSelect.addEventListener('change', () => {
            window.location = '{{ route('gec.schedules.create') }}?department=' + encodeURIComponent(departmentSelect.value);
        });
