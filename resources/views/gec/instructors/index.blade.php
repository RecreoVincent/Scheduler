...
<div class="page-header">
    ...
    <div class="actions">
        ...
        <a class="button button-secondary" href="{{ route('gec.instructors.import-template') }}">Download CSV Template</a>
...
<form class="filters" style="grid-template-columns:2fr 1fr 1fr;" method="GET" data-auto-filter>
    <div>
        <label for="instructorSearch">Search instructor</label>
        <input id="instructorSearch" class="input" name="search" value="{{ request('search') }}" placeholder="Name or email address">
    </div>
    <div>
        <label for="instructorEmploymentType">Employment type</label>
        <select id="instructorEmploymentType" class="input" name="employment_type">
    ...
    <div>
        ...
        <select id="instructorSort" class="input" name="sort">
        ...
        <table>
            ...
            @forelse ($instructors as $instructor)
                <tr>
                    ...
                    <td>
                        <div class="actions">
                            <a class="button button-secondary" href="{{ route('gec.instructors.index', array_merge(request()->query(), ['edit' => $instructor->id])) }}#instructorCreateModal">Edit</a>
                            <button type="button" class="button button-danger account-action" data-action="delete" data-url="{{ route('gec.instructors.destroy', $instructor) }}" data-name="{{ $instructor->name }}">Delete</button>
                        </div>
                    </td>
                </tr>
            @empty
    ...
    (() => {
        ...
        function closeModal() {
            @if($editingInstructor)
                window.location = '{{ route('gec.instructors.index', request()->except('edit')) }}';
                return;
            @endif
            modal.hidden = true;
...
<div id="instructorImportModal" class="admin-profile-modal" hidden>
    <section class="admin-profile-dialog" role="dialog" aria-modal="true" aria-labelledby="instructorImportTitle">
        ...
        <form method="POST" action="{{ route('gec.instructors.import') }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="admin-profile-form-grid">
                ...
                <div class="admin-profile-field full">
                    <p style="margin:0;color:var(--muted);font-size:12px;line-height:1.6">
                        ...
                        <a href="{{ route('gec.instructors.import-template') }}">Download a CSV template</a>.
                    </p>
                </div>
            </div>
