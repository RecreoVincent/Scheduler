...
<div class="form-grid">
    ...
    <div><label>Curriculum</label><select class="input" name="curriculum" required>@foreach(['New' => 'New Curriculum', 'Old' => 'Old Curriculum'] as $value => $label)<option value="{{ $value }}" @selected(old('curriculum', $subject->curriculum ?? 'New') === $value)>{{ $label }}</option>@endforeach</select></div>
