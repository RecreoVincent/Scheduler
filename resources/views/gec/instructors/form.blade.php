...
<form method="POST" action="{{ $editing ? route('gec.instructors.update', $instructor) : route('gec.instructors.store') }}">
    ...
    <div class="form-actions"><button class="button">{{ $editing ? 'Save Changes' : 'Add Instructor' }}</button><a class="button button-secondary" href="{{ route('gec.instructors.index') }}">Cancel</a></div>
</form>
</div>
@endsection
