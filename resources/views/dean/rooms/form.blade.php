...
<div class="card" style="max-width:650px">
    <form method="POST" action="{{ $editing ? route('dean.rooms.update', $room) : route('dean.rooms.store') }}">
        ...
        <div class="form-grid">
            ...
            <div><label for="room_type">Room Type</label><select id="room_type" class="input" name="room_type" required>@foreach($roomTypes as $type)<option value="{{ $type }}" @selected(old('room_type', $room->room_type ?? 'Lecture') === $type)>{{ $type }}</option>@endforeach</select></div>
        </div>
