    ...
    <form method="POST" action="{{ route('admin.users.store') }}">
        ...
        <div class="form-grid">
            ...
            <div class="form-group">
                ...
                <input id="middle_name" type="text" name="middle_name" value="{{ old('middle_name') }}" class="input" autocomplete="additional-name">
                @error('middle_name')<div class="error">{{ $message }}</div>@enderror
            </div>

            <div class="form-group">
                <label for="last_name">Last Name</label>
                <input id="last_name" type="text" name="last_name" value="{{ old('last_name') }}" class="input" autocomplete="family-name" required>
                @error('last_name')<div class="error">{{ $message }}</div>@enderror
            </div>

            <div class="form-group">
                <label for="suffix">Suffix <span style="font-weight:400;">(optional)</span></label>
                <input id="suffix" type="text" name="suffix" value="{{ old('suffix') }}" class="input" placeholder="Jr., Sr., III">
            ...
            <div class="form-group">
                ...
                <select id="employment_type" name="employment_type" class="input">
                    ...
                    <option value="full_time" @selected(old('employment_type') === 'full_time')>Full time (30 units maximum)</option>
                    ...
                    <option value="flexible_part_time" @selected(in_array(old('employment_type'), ['flexible_part_time', 'part_time'], true))>Flexible Part-Time (15 units maximum)</option>
                </select>
            ...
            <div class="form-group">
                ...
                <input id="outside_work_end_time" type="time" name="outside_work_end_time" class="input" value="{{ old('outside_work_end_time', '17:00') }}">
            ...
            <div class="form-group">
                ...
                <select id="year_level" name="year_level" class="input">
                    ...
                    @for ($level = 1; $level <= 4; $level++)
                        <option value="{{ $level }}" @selected(old('year_level') == $level)>Year {{ $level }}</option>
                    @endfor
                </select>
            ...
            <div class="form-group">
                ...
                <select id="academic_section_id" name="academic_section_id" class="input">
                    ...
                    @foreach ($sections as $section)
                        <option value="{{ $section->id }}" @selected(old('academic_section_id') == $section->id)>{{ $section->course }} · Year {{ $section->year_level }} · {{ $section->name }} · {{ $section->academic_year }}</option>
                    @endforeach
                </select>
