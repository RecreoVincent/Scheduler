...
<div class="page-header">
    ...
    <a class="button button-secondary" href="{{ route('gec.timetable.index') }}">Back to Timetables</a>
</div>
...
<div class="card" style="max-width:820px">
    ...
    <form method="POST" action="{{ route('gec.timetable.update', $timetable) }}">
        @csrf
        @method('PUT')
            ...
            <div>
                ...
                <select id="instructor_id" class="input" name="instructor_id" required>
                    @foreach($instructors as $instructor)
                        <option value="{{ $instructor->id }}" @selected((string) old('instructor_id', $timetable->instructor_id) === (string) $instructor->id)>{{ $instructor->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label for="room_id">Room</label>
                <select id="room_id" class="input" name="room_id">
                    <option value="">TBA (no room)</option>
                    @foreach($rooms as $room)
                        <option value="{{ $room->id }}" @selected((string) old('room_id', $timetable->room_id) === (string) $room->id)>{{ $room->course }} · {{ $room->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
        ...
        <div class="form-actions">
            ...
            <a class="button button-secondary" href="{{ route('gec.timetable.index') }}">Cancel</a>
        </div>
    </form>
</div>
@endsection
