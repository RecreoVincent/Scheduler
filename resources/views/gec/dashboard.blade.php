...
@section('page-title', 'GEC Dashboard')
...
<style>
    ...
    .schedule-row { display:grid; grid-template-columns:.7fr 1fr 1.4fr 1fr 1fr; gap:10px; padding:12px 0; border-bottom:1px solid #e2e8f0; font-size:13px; }
...
<div class="card">
    ...
    @forelse ($recentSchedules as $schedule)
        <div class="schedule-row"><span class="badge">{{ $schedule->course }}</span><strong>{{ $schedule->section?->name }}</strong><span>{{ $schedule->subject?->code }} · {{ $schedule->subject?->name }}</span><span>{{ $schedule->day }} {{ date('g:i A', strtotime($schedule->start_time)) }}</span><span>{{ $schedule->room?->name ?? 'TBA' }}</span></div>
    @empty
        <p>No minor-subject schedules generated yet.</p>
    @endforelse
</div>
    ...
    <section class="analytics-dialog" role="dialog" aria-modal="true" aria-labelledby="gecChartTitle">
        <div class="chart-header">
            ...
            <div class="chart-actions">
                ...
                <button type="button" id="closeGecAnalytics" class="chart-close" aria-label="Close analytics">&times;</button>
            </div>
        </div>
        <div id="gecChartCanvas" class="chart-layout"></div>
    </section>
</div>
@endsection
...
<script>
    (() => {
        ...
        const modal = document.getElementById('gecAnalyticsModal');
        const canvas = document.getElementById('gecChartCanvas');
        const title = document.getElementById('gecChartTitle');
        const closeButton = document.getElementById('closeGecAnalytics');
        ...
        function renderBar() {
            ...
            canvas.innerHTML = `<div class="bar-chart" role="img" aria-label="GEC analytics bar chart">${items.map(item => {
        ...
        function renderCircle() {
            ...
            if (!total) { canvas.innerHTML = '<p class="chart-empty">No GEC analytics are available yet.</p>'; return; }
            let current = 0;
            const segments = items.map(item => { const start=current; current += item.value/total*100; return `${item.color} ${start}% ${current}%`; }).join(',');
            canvas.innerHTML = `<div class="chart-circle ${chartType === 'doughnut' ? 'doughnut' : ''}" style="background:conic-gradient(${segments})" role="img" aria-label="GEC analytics ${chartType} chart"></div>${legend(items)}`;
        }
