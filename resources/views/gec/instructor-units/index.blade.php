...
@if($capacitySummary['unitShortfall'] > 0)
    ...
    <section class="unit-capacity-alert" role="alert">
        ...
        <div>
            ...
            <p>
                {{ $semester }} Semester Minor subjects across all departments require <strong>{{ $formatUnits($capacitySummary['totalSubjectUnits']) }} workload hours</strong>,
                while active GEC instructors can carry only <strong>{{ $formatUnits($capacitySummary['totalInstructorCapacity']) }} workload hours</strong>.
                ...
                @else
                    Set a positive full-time workload-hour default, then add enough GEC instructors to cover the shortfall before generating schedules.
                @endif
            </p>
        </div>
    </section>
                            ...
                            <td>
                                <div class="actions">
                                    <button
                                        ...
                                        data-update-url="{{ route('gec.instructor-units.update',$instructor) }}"
...
<div id="defaultUnitModal" class="admin-profile-modal" hidden>
    <section class="admin-profile-dialog" role="dialog" aria-modal="true" aria-labelledby="defaultUnitTitle">
        ...
        <form id="defaultUnitForm" method="POST" action="{{ route('gec.instructor-units.defaults') }}">
            @csrf
            @method('PATCH')
            <div class="admin-profile-field">
